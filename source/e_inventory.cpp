...
static void E_allocateInventoryItemIDs()
{
   ...
   while((item = runtime_cast<itemeffect_t *>(e_effectsTable.tableIterator(item))))
   {
      ...
      if(fxtype == ITEMFX_ARTIFACT)
      {
         ...
         // If the current item's sort order is the smallest thus far and is visible
         //else if(item->getInt(KEY_SORTORDER, 0) < e_minvisiblesortorder
         //        && item->getInt(KEY_INVBAR, 0))
         //   e_minvisiblesortorder = item->getInt(KEY_SORTORDER, 0);
